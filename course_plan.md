...
This session **introduces** functions rather than refactoring toward them. Monday's core path no
longer teaches `def`, so Wednesday cannot assume it. The vehicle is the repeated plotting code
students have actually written, which is also this day's contribution to the visualization spine.
...
- 64–72: six-question figure check, interpretation, and comparison with the annotated reference.
...
**Recovery:** `example_pier_frame()` supplies a provider-shaped teaching table so a failed Monday
...
**Go further:** `git restore --staged` and `commit --amend`; branches and pull requests; why notebook
JSON is hard to review and what the paired Jupytext source solves; conflict, secret, large-data, and
force-push scenarios discussed rather than performed.
