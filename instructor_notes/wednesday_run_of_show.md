...
Assumption: approximately 20 students, ten pairs, three instructors. Wednesday is restructured on
the novice-first pattern: every session is **worked example → core path → Go further**, and the core
path is genuinely finishable by someone who first opened a notebook on Monday.
...
- Monday's core no longer teaches functions, so Wednesday 1 must **introduce** them rather than
  refactor toward them. The vehicle is the repeated plotting code students have actually written,
  not the Pier loader.
...
- Session 1 no longer fails if Monday's download failed: `example_pier_frame()` provides a
...
| 64–72 | Six-question figure check; two pairs show a panel | Write one interpretation and one limitation | Open the annotated reference alongside |
